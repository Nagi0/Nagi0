...
<MyLayout>
    ...
    FloatLayout:
        ...
        Label:
            ...
            size_hint: 1, 0.1
            font_size: 40
            text: root.print_code(', '.join([str(symbol.data) for symbol in qrcodecam.symbols]))
"""
            ...
            if dd_mm_yy[0] >= 0:
                ...
                if dd_mm_yy[0] == 0:
                    ...
                    if abs(dd_mm_yy[1]) == 0:
                        ...
                        time.sleep(1.25)
                        if abs(dd_mm_yy[1]) == 1:
                            self.mes_sound = SoundLoader.load('Mês.wav')
                            self.mes_sound.play()
                            time.sleep(1)
                        else:
                            self.meses_sound = SoundLoader.load('Mêses.wav')
